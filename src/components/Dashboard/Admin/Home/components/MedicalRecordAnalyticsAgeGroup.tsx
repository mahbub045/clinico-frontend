...
import { useGetMedicalRecordAnalyticsAgeGroupQuery } from "@/redux/reducers/Admin/Home/MedicalRecordAnalyticsApi";
import { AgeGroupAnalyticsPoint } from "@/types/Admin/Home/DashboardDataTypes";
...
const getExtreme = (
  data: AgeGroupAnalyticsPoint[],
  key: keyof AgeGroupAnalyticsPoint,
...
const MedicalRecordAnalyticsAgeGroup: React.FC = () => {
  const { data, isLoading, isError } = useGetMedicalRecordAnalyticsAgeGroupQuery(undefined);
  const chartData: AgeGroupAnalyticsPoint[] = data ?? [];
  ...
  const bestSatisfactionGroup = hasData
    ...
    : null;
  const maxAverageSatisfaction = hasData
    ? Math.max(...chartData.map((item) => item.average_satisfaction))
    : 0;
    ...
    <section className="border-border/70 bg-card/90 space-y-6 rounded-[2rem] border p-4 shadow-sm shadow-slate-950/10 backdrop-blur-xl sm:p-6 dark:bg-slate-950/90 dark:shadow-black/10">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-3 max-w-2xl text-sm leading-6">
            Compare record volume, cost, and satisfaction across age groups.
          </p>
        </div>
      </div>
      ...
      {isLoading ? (
            ...
            {[
              ...
              },
              {
                ...
                value: highestCostGroup
                  ...
                  : "-",
                detail: highestCostGroup
                  ? highestCostGroup.age_group
                  : "",
                icon: DollarSign,
              },
              {
        ...
        <div className="border-destructive/10 bg-destructive/5 text-destructive rounded-[1.75rem] border p-6 text-sm">
          Unable to load age group analytics. Please refresh to try again.
        </div>
      ) : (
        <div className="border-border/70 bg-muted/80 text-muted-foreground rounded-[1.75rem] border p-6 text-sm dark:border-slate-800 dark:bg-slate-950/80">
          No age group analytics data available. Please check your filters or try again later.
        </div>
      )}
    </section>
  );
...
export default MedicalRecordAnalyticsAgeGroup;
