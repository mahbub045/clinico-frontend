...
import { useUpdateDoctorMutation } from "@/redux/reducers/Common/Doctors/DoctorsApi";
import {
  AddDoctorPayload,
...
const buildFormState = (values: RawDoctor): AddDoctorPayload => ({
  email: values.email ?? "",
  password: "",
  ...
  joined_date:
    values.joined_date === "" || values.joined_date === undefined
      ? null
      : values.joined_date,
  consultation_fee:
    values.consultation_fee === undefined || values.consultation_fee === null
      ? null
      : values.consultation_fee.toString(),
  chamber_room: values.chamber_room ?? "",
  experience_years:
...
const EditDioctorDialog: React.FC<EditDoctorDialogProps> = ({
  alias,
  initialValues,
  children,
}) => {
  const [open, setOpen] = useState(false);
  const [formData, setFormData] = useState<AddDoctorPayload>(
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await updateDoctor({ alias, doctorData: body }).unwrap();
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="max-h-[80vh] max-w-xl! overflow-y-auto shadow-md sm:max-w-3xl! md:max-w-4xl!">
        <DialogHeader>
          ...
          <DialogDescription>
            Update this doctor’s details and save your changes.
          </DialogDescription>
        </DialogHeader>
